...
function NftItem({
    ...
    try {
      ...
      if (address) {
        if (CurruntChain == chain) {
          ...
          const { request } = await prepareWriteContract({
            address: Contract_Addresss[0].nftMarketContractAddress,
            abi: Contract_Addresss[0].nftMarketContractAddress_Abi,
  ...
  const Add_to_favorite = async () => {
    try {
      if (!address) {
        ...
        let res = await axios.post("https://sanjhavehra.womenempowerment.online/Add_Favorite", {
          useraddress: address,
          itemId: NFT.itemId,
  ...
  const update_Favorite = async () => {
    try {
      let res = await axios.post(`https://sanjhavehra.womenempowerment.online/update_Favorite`, {
        name: NFT.name,
        useraddress: address
      })
  ...
  const get_Favorite = async () => {
    try {
      ...
      let res = await axios.get(`https://sanjhavehra.womenempowerment.online/get_One_Favorite?useraddress=${address}&&name=${NFT.name}`)
  ...
  return (
    <>
      <div className={`card rounded shadow-sm card-hover-image ${category}`}>
        ...
        <div className="card-body p-3 position-relative">
          ...
          <div className="favroute" onClick={() => (favrouteName != NFT.name ? (setIsFavorite(!IsFavorite), Add_to_favorite()) : update_Favorite())}>
            <div className="inner_blockchain">
              <div className="like_hrt"  >
                {
                  favrouteName == NFT.name ? <> <MdFavorite style={{ color: "red" }} /> </> : <> <MdFavoriteBorder /></>
                }
                ...
                <>
                  <div ref={vidButtonRef} class="c-mm">
                    ...
                    <Link className='text-reset' to={`/assets/${NFT.name}/${NFT.Blockchain}`} >
                      ...
                      <video src={NFT.url} ref={vidRef} loop width="100%" style={{ height: "20rem" }}  ></video>
                    </Link>
                  </div>
